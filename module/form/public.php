...
class FormHandler extends ModuleHandler
{
	...
	function Form2Email()
	{
		...
		if ($request->GetProperty('FormSubmitted') == 'Yes')
		{
			...
			if($request->GetProperty('PrivacyPolicyAgreement') != "Y")
			{
				$errors[] = array("Message" => GetTranslation('privacy-policy-agreement-required'));
			}
			...
			else if (count($emailFields) > 0)
			{
				...
				{
					if($cityList->_items[$i]["Selected"])
						$formEmail = $cityList->_items[$i]["FormEmail"];
				}
				
				$result = SendMailFromAdmin($formEmail, $this->config['Subject'], $emailTemplate->Grab($email), $uploadedFiles);
