...
class InfoblockItemList extends LocalObjectList
{
	...
	function _PrepareContentBeforeShow()
	{
		for ($i = 0; $i < count($this->_items); $i++)
		{	
			$this->_items[$i]['Year'] = date('Y', strtotime( $this->_items[$i]['ItemDate']));
			$this->_items[$i]['Month'] = LocalDate('F', strtotime( $this->_items[$i]['ItemDate']));
			$this->_items[$i]['Day'] = date('d', strtotime( $this->_items[$i]['ItemDate']));
			$this->_items[$i]['ItemDateRu'] = date('d.m.Y', strtotime( $this->_items[$i]['ItemDate']));
