	...
	function ShowHTML()
	{
		...
		switch ($request->GetProperty('View'))
		{
			case 'CategoryList':
				...
				if ($this->config["HasCategoryListTemplate"] == 0)
				{
					header("Location: ".$categoryList->_items[0]["CategoryURL"]);
					exit();
				}
				
				for($i = 0; $i < $categoryList->GetCountItems(); $i++)
				{
					$req = clone ($request);
					$req->SetProperty("ViewCategoryID", $categoryList->_items[$i]["CategoryID"]);
					$itemList = new InfoblockItemList($this->module, $this->config);
					$itemList->SetPageParam("CategoryPage".$categoryList->_items[$i]["CategoryID"]);
					$itemList->Load($req);
					$categoryList->_items[$i]["ItemList"] = $itemList->GetItems();
					
					$url = $request->GetProperty('BaseURL')."".HTML_EXTENSION."?CategoryID=".$categoryList->_items[$i]["CategoryID"];
					$urlFirstPage = $request->GetProperty('BaseURL')."".HTML_EXTENSION."?CategoryID=".$categoryList->_items[$i]["CategoryID"];
					$categoryList->_items[$i]["Paging"] = $itemList->GetPagingAsArray($url, $urlFirstPage, $categoryList->_items[$i]["CategoryID"]);
					if($request->GetProperty("CategoryID"))
						$categoryList->_items[$i]["Selected"] == true;
					$categoryList->_items[$i]["CurrentPage"] = $itemList->GetCurrentPage();
				}
