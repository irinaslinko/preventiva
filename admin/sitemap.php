...
if ($request->GetProperty("Save") == 1)
{
	...
	{
		if($currentDomain)
			unlink(PROJECT_DIR."website/".WEBSITE_FOLDER."/".$currentDomain.".sitemap.xml");
		else
	...
	else 
	{
		...
		if($currentDomain)
			$fp = fopen(PROJECT_DIR."website/".WEBSITE_FOLDER."/".$currentDomain.".sitemap.xml", "w");
		else
...
{
	if($currentDomain)
		$sitemapContent = file_get_contents(PROJECT_DIR."website/".WEBSITE_FOLDER."/".$currentDomain.".sitemap.xml");
	else
