...
class AdminPage extends LocalPage
{
	...
	function _InitHeader($header, $pageID)
	{
		...
		$adminMenu[] = array(
			"Title" => GetTranslation("admin-menu-city"),
			"Link" => "city.php",
			"AdminMenuIcon" => "fa fa-building-o"
		);
...
class PublicPage extends LocalPage
{
	...
	function _CreateTemplate($file, $header, $pageID)
	{
		...
		$cityList->LoadCityList(true);
		for($i = 0; $i < $cityList->GetCountItems(); $i++)
		{
			$branchList = new BranchList();
			$branchList->LoadBranchListByID($cityList->_items[$i]["CityID"], true);
			$cityList->_items[$i]["BranchList"] = $branchList->GetItems();
			for ($j = 0; $j < $branchList->GetCountItems(); $j++)
			{
				$branch = new Branch();
				$branch->LoadByID($branchList->_items[$j]["BranchID"]);
				$hourList = $branch->GetHourListByBranchID($branchList->_items[$j]["BranchID"]);
				$cityList->_items[$i]["BranchList"][$j]["HourList"] = $hourList;
			}
		}
		$tmpl->SetLoop("CityList", $cityList->GetItems());
		return $tmpl;
	}
	...
	function _InitHeader($header, $pageID)
	{
		...
		$this->headerTmpl->SetLoop("CityList", $cityList->GetItems());
	}
	...
	function _InitFooter($header, $pageID)
	{
		...
		for($i = 0; $i < $cityList->GetCountItems(); $i++)
		{
			...
			{
				$branch = new Branch();
				$phoneList = $branch->GetPhoneListByBranchID($branchList->_items[$j]["BranchID"]);
				$cityList->_items[$i]["BranchList"][$j]["PhoneList"] = $phoneList;
			}
		}
		$this->footerTmpl->SetLoop("CityList", $cityList->GetItems());
	}
}
